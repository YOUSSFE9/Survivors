...
export default function SquadLobby({ uid, playerName, avatarUrl = '', onBack, onMatchFound, t }: Props) {
    ...
    const handleConnect = async (flow: SquadFlow) => {
        ...
        try {
            ...
            if (flow === 'create_red' || flow === 'create_blue') {
                ...
                if (!full) { setStatus(t.enterCodeFirst); setBusy(false); return; }
            ...
            <div style={S.overlay}>
                <div style={S.card}>
                    <div style={S.header}>
                        ...
                        <h2 style={{ margin: '8px 0 4px', fontSize: 20 }}>{t.lbSquadTitle}</h2>
                        <p style={S.sub}>{status || t.waitingOthers}</p>
                    </div>
                    {(redCode || blueCode) && (
                        <div style={{ display: 'flex', gap: 10, marginBottom: 18, flexWrap: 'wrap' }}>
                            {redCode && <CodeBox label={t.lbRedTeam} code={redCode} color="#ff4455" t={t} />}
                            {blueCode && <CodeBox label={t.lbBlueTeam} code={blueCode} color="#4488ff" t={t} />}
                        </div>
                    )}
                    <div style={S.playerSection}>
                        <div style={S.sectionTitle}>{t.players} ({players.length})</div>
                        <div style={{ display: 'flex', gap: 14 }}>
                            <TeamColumn title={t.lbRedTeam} players={redPlayers} color="#ff4455" myName={playerName} isHost={isHost} room={roomRef} t={t} />
                            <TeamColumn title={t.lbBlueTeam} players={bluePlayers} color="#4488ff" myName={playerName} isHost={isHost} room={roomRef} t={t} />
                        </div>
                    </div>
                    {isHost && countdown == null && (
                        <div style={S.hostMsg}>
                            <div style={{ color: '#44ffaa', fontSize: 12, marginBottom: 10 }}>👑 {t.lbYouAreHost}</div>
                            <button style={{ ...S.btn, background: '#22aa66', width: '100%' }} onClick={() => roomRef?.send('host_start')}>🚀 {t.lbStartGame}</button>
                        </div>
                    )}
                    <button style={{ ...S.btn, background: '#222', width: '100%', marginTop: 10 }} onClick={handleLeave}>{t.lbLeaveBack}</button>
    ...
    return (
        <div style={{ ...S.overlay, direction: t.dir || 'rtl' }}>
            <div style={S.card}>
                <div style={S.header}>
                    <h1 style={{ margin: 0, fontSize: 24 }}>🛡️ {t.lbSquadTitle}</h1>
                    <p style={S.sub}>{t.lbSquadDesc}</p>
                </div>
                {squadFlow === 'none' && (
                    <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
                        <button style={{ ...S.btn, background: '#6b0a0a', flex: 1 }} onClick={() => { setSquadFlow('create_red'); handleConnect('create_red'); }}>{t.lbRedTeam}</button>
                        <button style={{ ...S.btn, background: '#0a1a6b', flex: 1 }} onClick={() => { setSquadFlow('create_blue'); handleConnect('create_blue'); }}>{t.lbBlueTeam}</button>
                        <button style={{ ...S.btn, background: '#1a4a1a', flex: 1 }} onClick={() => setSquadFlow('join')}>{t.back}</button>
                    </div>
                )}
                {squadFlow === 'join' && (
                    <div style={{ marginBottom: 14 }}>
                        <input style={S.input} placeholder={t.lbSquadCodePlaceholder} value={joinCode} onChange={e => setJoinCode(e.target.value)} maxLength={30} autoFocus />
                    </div>
                )}
                <div style={S.btnRow}>
                    <button style={{ ...S.btn, background: '#333', flex: 1 }} onClick={onBack}>{t.back}</button>
                    {squadFlow === 'join' && (
                        <button style={{ ...S.btn, background: '#4488ff', flex: 2 }} onClick={() => handleConnect('join')} disabled={busy}>{t.lbJoinBtn}</button>
                    )}
                </div>
...
function TeamColumn({ title, players, color, myName, isHost, room, t }: any) {
    return (
        <div style={{ flex: 1 }}>
            <div style={{ color, fontWeight:700, fontSize:13, marginBottom:8 }}>{title} ({players.length})</div>
            {players.length === 0
                ? <div style={{ color:'rgba(255,255,255,0.2)', fontSize:12, padding:8 }}>{t.waitingOthers}</div>
                : players.map((p: any) => (
                    <div key={p.sessionId} style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 10px', borderRadius:8, marginBottom:4, background: p.name === myName ? 'rgba(68,255,170,0.1)' : 'rgba(255,255,255,0.03)' }}>
                        ...
                        <span style={{ flex:1, fontSize:13, color: p.name === myName ? '#44ffaa' : '#cdd' }}>{p.name} {p.name === myName && `(${t.you})`}</span>
